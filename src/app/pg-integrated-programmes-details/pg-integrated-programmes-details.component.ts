...
export class PgIntegratedProgrammesDetailsComponent implements OnInit {
      ...
      .subscribe({
        next: (data: any[]) => {
          if (data && data.length > 0) {
            ...
            this.GetIntegratedPgProgramsData.forEach((program: any) => {
    ...
    this.apiService.getAllProgramMetas(this.sCourseCode).subscribe({
      next: (data: any[]) => {
        ...
        if (pageData) {
          ...
          const program =
            this.GetIntegratedPgProgramsData &&
            this.GetIntegratedPgProgramsData.length > 0
              ? this.GetIntegratedPgProgramsData[0]
              : null;

          const title =
            pageData.Title || program?.sfullname || 'PG Integrated Program';

          const description =
            pageData.Description ||
            'An integrated postgraduate program combining undergraduate and postgraduate studies with industry exposure.';
  ...
  injectStructuredData(pageData: any): void {
    ...
    const pageUrl =
      ...
      `${baseUrl}/pg-integrated-programmes/${this.sCourseCode}`;
      ...
      '@graph': [
        {
          ...
          '@id': pageUrl,
          url: pageUrl,
          name: pageData.Title || program?.sfullname || 'PG Integrated Program',
          description:
            pageData.Description ||
            'An integrated postgraduate program combining advanced academic learning with practical exposure.',
          programType: 'Integrated Postgraduate Program',
          provider: { '@id': `${baseUrl}#college` },
          hasCourse: { '@id': `${pageUrl}#course-details` },
          occupationalCredentialAwarded:
            program?.DegreeName || 'Integrated Postgraduate Degree',
...

        {
          ...
          name: program?.CourseName || 'Integrated Program Curriculum',
          description:
            'Structured curriculum covering undergraduate and postgraduate level coursework.',
          provider: { '@id': `${baseUrl}#college` },
        },

        {
          ...
          foundingDate: '2005',
          description:
            'A premier university offering integrated undergraduate and postgraduate programs with global exposure.',
          address: {
            '@type': 'PostalAddress',
