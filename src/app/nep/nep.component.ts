...
@Component({
  ...
  styleUrl: './nep.component.css',
})
export class NepComponent {
  constructor(
    private meta: Meta,
  ...
  updateMetaTags(): void {
    this.apiService.getNepMeta().subscribe({
      next: (data: any[]) => {
        if (Array.isArray(data) && data.length > 0) {
          ...
          const title = metaData.Title || 'NEP – Amity University Noida';

          const description =
            metaData.Description ||
            'Learn about the National Education Policy (NEP) implementation at Amity University Noida, including flexible learning, multidisciplinary education, and academic innovation.';
