...
export class PgIndustryProgramDetailsComponent implements OnInit {
      ...
      .subscribe({
        next: (data: any[]) => {
          if (data?.length > 0) {
            ...
            // IMPORTANT:
            // Generate meta + schema only after program data is loaded
    ...
    this.apiService.getAllProgramMetas(this.sCourseCode).subscribe({
      next: (data: any[]) => {
        ...
        const programName =
          ...
          pageData?.Title ||
          'Industry Integrated PG Programme';

        const title = pageData?.Title || programName;

        const description =
          pageData?.Description ||
          program?.Description ||
          program?.CourseDescription ||
          'Explore industry-integrated postgraduate programmes at Amity University Noida with strong corporate collaboration and real-world exposure.';

        let imageUrl = 'https://noida.amity.edu/assets/img/breadcrump_bg.jpg';
      ...
      error: (error: any) => {
        console.error('Error fetching PG program meta data:', error);
      },
    });
  }
  ...
  injectStructuredData(pageData: any): void {
    ...
    if (!program) {
      console.warn('Industry PG program data not available for schema.');
      return;
    }
    ...
    const description = this.stripHtml(
      pageData?.Description ||
        ...
        program?.CourseDescription ||
        program?.sshortdesc ||
        `Explore ${programName}, an industry-integrated postgraduate programme offered by Amity University Noida.`,
    );
    ...
    // DISCIPLINE
    // =====================================================
    const disciplineName = this.stripHtml(
    ...
    const programmeSchema: any = {
      ...
      programType: 'Industry Integrated Postgraduate degree programme',
