...
@Component({
  selector: 'app-student-innovation',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './student-innovation.component.html',
  styleUrl: './student-innovation.component.css',
})
export class StudentInnovationComponent implements OnInit {
  studentList: any[] = [];
  ...
  getData() {
    this.apiService.GetAcaiStudentInnovation().subscribe({
      next: (res: any[]) => {
        this.studentList = (res || []).slice(0, 6).map((item) => ({
          ...item,

          ImagePath1: item.ImagePath1 || item.Imagepath1 || '',

          ImagePath: item.ImagePath || '',
          ImagePath2: item.ImagePath2 || '',
          ImagePath3: item.ImagePath3 || '',
          ImagePath4: item.ImagePath4 || '',
          ImagePath5: item.ImagePath5 || '',

          StudentName2: item.StudentName2 || '',
          StudentName3: item.StudentName3 || '',
          StudentName4: item.StudentName4 || '',
          StudentName5: item.StudentName5 || '',
        }));
      },
  ...
  updateMetaTags(): void {
    this.apiService.getAcaiStudentInnovationMeta().subscribe({
      next: (data: any[]) => {
        if (Array.isArray(data) && data.length > 0) {
          ...
          const title =
            metaData.Title || 'Student Innovation – Amity University Noida';

          const description =
            metaData.Description ||
            'Explore student innovation at Amity University Noida including projects, startups, research ideas, and creative achievements by students.';
    ...
    const schema = {
      ...
      '@graph': [
        /* ================= PAGE ================= */
        {
          ...
          '@id': canonicalUrl,
          url: canonicalUrl,
          name: metaData.Title || 'Student Innovation – Amity University Noida',
          description:
            metaData.Description ||
